...
if __name__ == '__main__' :
    sick_name = 'is_Kidney'
    py_data = 'bf'
    path = './data/figures/figure_Integration_{}_before.jpg'.format(sick_name)
    lw = 3
    ...
    for fold in range(0,5):
        ...
        patients_ending_valid_NLP = torch.load('./data/Integration_data/patients_valid_NLP_{}_{}-{}.pt'.format(sick_name,py_data,str(fold)))
        logits_NLP = torch.load('./data/Integration_data/logits_NLP_{}_{}-{}.pt'.format(sick_name,py_data,str(fold)))

        patients_ending_valid_CT = torch.load('./data/Integration_data/patients_valid_CT_{}_{}-{}.pt'.format(sick_name, py_data, str(fold)))
        logits_CT = torch.load('./data/Integration_data/logits_CT_{}_{}-{}.pt'.format(sick_name, py_data, str(fold)))
        ...
        patients_ending_valid_inte = torch.load('./data/Integration_data/patients_valid_{}_{}_{}.pt'.format(sick_name,py_data,str(fold)))
        logits_inte = torch.load('./data/Integration_data/logits_{}_{}_{}.pt'.format(sick_name,py_data,str(fold)))
    ...
    plt.plot(fpr_CT, tpr_CT, color='darkorange', lw=lw, label='CT_Kidney_before (AUC = %0.3f)' % roc_auc_CT)
    plt.plot(fpr, tpr, color='darkorchid', lw=lw, label='inte_Kidney_before (AUC = %0.3f)' % roc_auc)
    plt.plot([0, 1], [0, 1], color='silver', lw=lw, linestyle='--')

    plt.plot(fpr_NLP, tpr_NLP, color='cyan', lw=lw, label='NLP_Kidney_before (AUC = %0.3f)' % roc_auc_NLP)

    plt.plot(fpr_MR, tpr_MR, color='crimson', lw=lw, label='MR_Kidney_before (AUC = %0.3f)' % roc_auc_MR)
    plt.plot(fpr_inte, tpr_inte, color='dodgerblue', lw=lw, label='CF_Kidney_before (AUC = %0.3f)' % roc_auc_inte)
