...
if __name__ == '__main__' :
    sick_name = 'is_lung'
    path = './data/figures/figure_Integration_lung.jpg'
    py_type = 'multi'
    lw = 3
    # flag = True
    for fold in range(0,5):
        ...
        patients_ending_valid_inte_lung = torch.load( './data/Integration_data/patients_valid_is_lung_{}_{}.pt'.format(py_type,str(fold)))
        logits_valid_inte_lung = torch.load('./data/Integration_data/logits_is_lung_{}_{}.pt'.format(py_type, str(fold)))
    ...
    plt.plot(fpr_inte_lung, tpr_inte_lung, color='darkorange', lw=lw,
             label='inte_lung (AUC = %0.3f)' % roc_auc_inte_lung)
    fpr_NLP, tpr_NLP, roc_auc_NLP = caculate_auc(NLP_label, NLP_logits)
    print(roc_auc_NLP)
    plt.plot(fpr_NLP, tpr_NLP, color='cyan', lw=lw, label='NLP_lung (AUC = %0.3f)' % roc_auc_NLP)
    plt.plot(fpr, tpr, color='darkorchid', lw=lw, label='CF_lung (AUC = %0.3f)' % roc_auc)

    fpr_CT, tpr_CT, roc_auc_CT = caculate_auc(CT_label, CT_logits)
    print(roc_auc_CT)
    plt.plot(fpr_CT, tpr_CT, color='dodgerblue', lw=lw, label='CT_lung (AUC = %0.3f)' % roc_auc_CT)
